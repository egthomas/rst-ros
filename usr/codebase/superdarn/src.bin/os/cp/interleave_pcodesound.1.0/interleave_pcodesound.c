...
char progid[80]={"interleave_pcodesound 2026/03/09"};
char progname[256];
...
int rst_opterr(char *txt) {
  ...
  fprintf(stderr,"Please try: interleave_pcodesound --help\n");
  return(-1);
}
...
int main(int argc,char *argv[])
{
  ...
  sprintf(progname,"interleave_pcodesound (fast)");
  for (i=0; i<nintgs; i++){
  ...
  total_integration_usecs = total_scan_usecs/nintgs;
...
void usage(void)
{
    printf("\ninterleave_pcodesound [command-line options]\n\n");
