...
void SiteFheExit(int signum) {
  ...
  switch(signum) {
    case 2:
      ...
      printf("SiteFheExit: Sig 2: %d\n",FHE_exit_flag); 
      if (cancel_count < 3 )
        break;
    case 0:
      printf("SiteFheExit: Sig 0: %d\n",FHE_exit_flag); 
      if(FHE_exit_flag!=0) {
        msg.type=QUIT;
...
/* total_samples*8 represents number of bytes for main and back samples */
      ...
      if (debug) {
        ...
        fprintf(stderr,"FHE seq %d :: rdata.main 16bit 8-11: %8d %8d %8d %8d\n",nave,
...
*/
   ...
   if (debug) {
     ...
     fprintf(stderr,"FHE SiteIntegrate: end\n");
   }
   SiteFheExit(0);
   return nave;
}
