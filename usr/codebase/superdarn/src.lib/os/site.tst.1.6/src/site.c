...
int SiteTstStartIntt(int sec,int usec) {
  ...
  SiteTstExit(0);
  if (debug) {
    fprintf(stderr,"SiteTstStartInt: start\n");
  }
  ...
  if (debug) {
    fprintf(stderr,"SiteTstStartInt: end\n");
  }
...
*/
    ...
    TCPIPMsgRecv(sock, &rmsg, sizeof(struct ROSMsg));
    if (debug) {
      fprintf(stderr,"TST GET_DATA:type=%c\n",rmsg.type);
      fprintf(stderr,"TST GET_DATA:status=%d\n",rmsg.status);
    }
    ...
    TCPIPMsgRecv(sock, &rmsg, sizeof(struct ROSMsg));
    if (debug) {
      fprintf(stderr,"TST GET_PARAMETERS:type=%c\n",rmsg.type);
      fprintf(stderr,"TST GET_PARAMETERS:status=%d\n",rmsg.status);
      fprintf(stderr,"TST Number of samples: dprm.samples:%d tsprm.samples:%d total_samples:%d\n",dprm.samples,tsgprm.samples,total_samples);
      fprintf(stderr,"TST nave=%d\n",nave);
    }
...
/* total_samples*8 represents number of bytes for main and back samples */
      ...
      if (debug) {
        fprintf(stderr,"TST seq %d :: ioff: %8d\n",nave,iqoff);
        fprintf(stderr,"TST seq %d :: rdata.main 16bit 8-11: %8d %8d %8d %8d\n",nave,
           ((int16 *)rdata.main)[8],((int16 *)rdata.main)[9],
           ((int16 *)rdata.main)[10],((int16 *)rdata.main)[11]);
        dest = (void *)(samples);
        dest += iqoff;
        fprintf(stderr,"TST seq %d :: samples    16bit 8-11: %8d %8d %8d %8d\n",nave,
           ((int16 *)dest)[8],((int16 *)dest)[9],
           ((int16 *)dest)[10],((int16 *)dest)[11]);
        fprintf(stderr,"TST seq %d :: rdata.back 16bit 8-11: %8d %8d %8d %8d\n",nave,
        ...
        fprintf(stderr,"TST seq %d :: samples    16bit 8-11: %8d %8d %8d %8d\n",nave,
           ((int16 *)dest)[8],((int16 *)dest)[9],
           ((int16 *)dest)[10],((int16 *)dest)[11]);
        fprintf(stderr,"TST seq %d :: iqsze: %8d\n",nave,iqsze);
      }
    ...
    /* calculate ACF */   
      if (mplgexs==0) {
        ...
        rngoff=2*rxchn; 
        if (debug) 
        fprintf(stderr,"TST seq %d :: rngoff %d rxchn %d\n",nave,rngoff,rxchn);
        if (debug) 
        fprintf(stderr,"TST seq %d :: ACFSumPower\n",nave);
        ...
        fprintf(stderr,"TST seq %d :: rngoff %d rxchn %d\n",nave,rngoff,rxchn);
        if (debug) 
        fprintf(stderr,"TST seq %d :: ACFCalculate acf\n",nave);
        ...
        fprintf(stderr,"TST seq %d :: rngoff %d rxchn %d\n",nave,rngoff,rxchn);
        if (debug) 
          fprintf(stderr,"TST seq %d :: ACFCalculate xcf\n",nave);
          ACFCalculate(&tsgprm,(int16 *) dest,rngoff,skpnum!=0,
        ...
        fprintf(stderr,"TST seq %d :: rngoff %d rxchn %d\n",nave,rngoff,rxchn);
        if (debug) 
          fprintf(stderr,"TST seq %d :: ACFNormalize\n",nave);
...
*/
   ...
   SiteTstExit(0);
   if (debug) {
     fprintf(stderr,"TST SiteIntegrate: iqsize in bytes: %ld in 16bit samples:  %ld in 32bit samples: %ld\n",iqsze,iqsze/2,iqsze/4);
     fprintf(stderr,"TST SiteIntegrate: end\n");
   }
   return nave;
}
